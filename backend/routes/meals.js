...
router.put('/:id', auth, [
  ...
  try {
    ...
    if (items) {
      ...
      for (const item of items) {
        console.log('UPDATE - Processing item:', { foodId: item.food, weight: item.weight });
        ...
        console.log('UPDATE - Found food:', { name: food.name, nameAr: food.nameAr, servingSize: food.servingSize });
        ...
        try {
          // Calculate nutrition for the specified weight
          const nutrition = food.calculateNutrition(item.weight);
          console.log('UPDATE - Calculated nutrition:', nutrition);
...
router.post('/quick-add', auth, [
  ...
  body('weight').isFloat({ min: 1, max: 5000 }).withMessage('Weight must be between 1 and 5000 grams'),
  body('mealType').isIn(['breakfast', 'lunch', 'dinner', 'snack']).withMessage('Invalid meal type'),
  body('date').optional().isISO8601().withMessage('Invalid date format')
], async (req, res) => {
  try {
