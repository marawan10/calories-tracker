...
class GoogleFitService {
      ...
      return new Promise((resolve, reject) => {
        ...
        const messageListener = (event) => {
          ...
          if (event.data.type === 'GOOGLE_AUTH_SUCCESS') {
            ...
            this.storeToken(this.accessToken, this.tokenExpiry);
  ...
  async signOut() {
    ...
    try {
      ...
      // Successfully signed out from Google Fit
    } catch (error) {
  ...
  loadStoredToken() {
    try {
      const storedToken = localStorage.getItem('googleFit_accessToken');
      const storedExpiry = localStorage.getItem('googleFit_tokenExpiry');
      ...
      if (storedToken && storedExpiry) {
        ...
        if (this.isTokenValid()) {
          this.isSignedIn = true;
          // Loaded valid stored Google Fit token
        } else {
          // Stored Google Fit token expired, clearing
          this.clearStoredToken();
        }
      }
    } catch (error) {
  ...
  storeToken(accessToken, expiryTime) {
    try {
      localStorage.setItem('googleFit_accessToken', accessToken);
      localStorage.setItem('googleFit_tokenExpiry', expiryTime.toString());
      localStorage.setItem('googleFit_connected', 'true');
      // Stored Google Fit token
    } catch (error) {
  ...
  clearStoredToken() {
    try {
      ...
      localStorage.removeItem('googleFit_tokenExpiry');
      localStorage.removeItem('googleFit_connected');
      this.accessToken = null;
      this.tokenExpiry = null;
      this.isSignedIn = false;
      // Cleared stored Google Fit token
    } catch (error) {
  ...
  async getFitnessData(startDate, endDate) {
    ...
    try {
      ...
      // Received fitness data response
  ...
  processFitnessData(rawData) {
    // Processing raw fitness data
    const processedData = [];

    if (!rawData.bucket || !Array.isArray(rawData.bucket)) {
      // No bucket data found in response
