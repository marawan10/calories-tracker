...
const GoogleFitIntegration = ({ onDataSync, onConnectionChange }) => {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      if (!document.hidden) {
        // Page visible - syncing Google Fit data
        await loadTodaysFitData();
      }
    };
  ...
  const initializeGoogleFit = async () => {
    try {
      ...
      // Debug logging
      // Google Fit configuration loaded
  ...
  const connectToGoogleFit = async () => {
    try {
      ...
      // Attempting to sign in to Google Fit
  ...
  const loadTodaysFitData = async () => {
    try {
      ...
      if (error.message.includes('401')) {
        // 401 error - token might be invalid, disconnecting
        setIsConnected(false);
  ...
  const syncFitnessData = async (days = 7) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Successfully synced fitness data
      }
